// app/(your-page)/FormAllocateTable.jsx
"use client";
...
export default function FormAllocateTable() {
  // ---- state หลัก
  ...
  const [sort, setSort] = useState("code-asc");     // code-asc|code-desc|title-asc|title-desc|start-asc|start-desc|end-asc|end-desc|id-asc|id-desc
  const [page, setPage] = useState(1);              // หน้าปัจจุบัน
  const [size, setSize] = useState(5);              // จำนวนต่อหน้า
  const [showTable, setShowTable] = useState(true);

  // ---- โหลดข้อมูล form_allocate
  useEffect(() => {
    let alive = true;
    setLoading(true);
    apiGet("/api/form_allocate")
      .then((data) => {
        if (!alive) return;
        const list = Array.isArray(data) ? data : data?.data || [];
        // normalize เป็นรูปแบบเดียว
        const normalized = list.map((d, i) => ({
          id: d.form_allocate_id ?? i + 1,
          code: d.allocate_code ?? "-",
          title: d.report_title_th ?? d.title ?? "-",
          start_date: d.start_date ?? null,
          end_date: d.end_date ?? null,
          form_utilization_id: d.form_utilization_id ?? null,
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input
                ...
                placeholder="ค้นหา ชื่อเรื่อง / เลขที่รอบจัดสรร / รหัส…"
              ...
              >
                <optgroup label="เรียงตามชื่อ/โค้ด">
                  ...
                  <option value="code-asc">เลขที่ (A→Z)</option>
                  <option value="code-desc">เลขที่ (Z→A)</option>
                </optgroup>
              ...
              <table className="min-w-full bg-white text-sm">
                <thead>
                  <tr className="bg-black/[.03] text-left text-black/60">
                    ...
                    <th className="px-4 py-2 font-medium">เลขที่รอบจัดสรร</th>
