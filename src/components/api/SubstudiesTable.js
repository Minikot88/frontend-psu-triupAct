...
export default function SubstudiesTable() {
  ...
  return (
    <Card>
      <CardHeader title="ตารางสาขาย่อย /api/substudies" onToggle={()=>setShowTable(!showTable)} isOpen={showTable} />
      {showTable && (
        <>
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input value={q} onChange={(e)=>{setQ(e.target.value);setPage(1);}} placeholder="ค้นหา ชื่อสาขาย่อย/ID…" className="w-80 rounded-xl border border-black/15 bg-white px-3 py-2 text-sm outline-none" />
              ...
              <table className="min-w-full bg-white text-sm">
                <thead>
                  <tr className="bg-black/[.03] text-left text-black/60">
                    ...
                    <th className="px-4 py-2 font-medium">ชื่อสาขาย่อย</th>
                  </tr>
                </thead>
