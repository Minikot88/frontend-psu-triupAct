...
export default function GroupstudiesTable() {
  ...
  useEffect(() => {
    ...
    apiGet("/api/groupstudies")
      .then((data) => {
        if (!alive) return;

        // API ตัวอย่างส่ง { status, data: [...] }
        // แต่กันไว้สำหรับกรณีส่งเป็น object keyed-by-id หรือ object เดี่ยว
  ...
  const filtered = useMemo(() => {
    ...
    out.sort((a, b) => {
      if (sort.startsWith("title")) {
        const cmp = String(a.title ?? "").localeCompare(
          String(b.title ?? ""),
          "th",
          {
            sensitivity: "base",
  ...
  return (
    <Card>
      <CardHeader
        title="ตารางกลุ่มสาขาการศึกษา /api/groupstudies"
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input
                ...
                placeholder="ค้นหา ชื่อกลุ่มสาขา หรือ ID…"
