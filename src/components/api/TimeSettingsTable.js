...
export default function TimeSettingsTable() {
  ...
  return (
    <Card>
      <CardHeader title="ตารางเงื่อนไขระยะเวลา /api/time_settings" onToggle={()=>setShowTable(!showTable)} isOpen={showTable} />
      {showTable && (
        <>
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input value={q} onChange={(e)=>{setQ(e.target.value);setPage(1);}} placeholder="ค้นหา ข้อความ/ID…" className="w-96 rounded-xl border border-black/15 bg-white px-3 py-2 text-sm outline-none" />
              <select value={sort} onChange={(e)=>setSort(e.target.value)} className="rounded-xl border border-black/15 bg-white px-3 py-2 text-sm outline-none">
                ...
                <option value="title-asc">ข้อความ (A→Z)</option>
                <option value="title-desc">ข้อความ (Z→A)</option>
              </select>
