...
export default function PrefixsTable() {
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");

  const [q, setQ] = useState("");                 // คำค้นหา
  const [sort, setSort] = useState("prefix-asc"); // เรียงชื่อคำนำหน้า
  ...
  const filtered = useMemo(() => {
    ...
    let out = rows.filter(
      (r) =>
        !term ||
        String(r.prefix ?? "").toLowerCase().includes(term) ||
        String(r.id ?? "").toLowerCase().includes(term)
    );

    out.sort((a, b) => {
      if (sort.startsWith("prefix")) {
        const cmp = String(a.prefix ?? "").localeCompare(String(b.prefix ?? ""), "th", {
          sensitivity: "base",
        });
        return sort === "prefix-asc" ? cmp : -cmp;
  ...
  return (
    <Card>
      <CardHeader
        title="ตารางคำนำหน้าชื่อ /api/prefixs"
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input
                ...
                placeholder="ค้นหา คำนำหน้า หรือ ID…"
              ...
              >
                <option value="prefix-asc">ชื่อคำนำหน้า (A→Z)</option>
                <option value="prefix-desc">ชื่อคำนำหน้า (Z→A)</option>
              ...
              <table className="min-w-full bg-white text-sm">
                <thead>
                  <tr className="bg-black/[.03] text-left text-black/60">
                    ...
                    <th className="px-4 py-2 font-medium">คำนำหน้า</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-black/10">
                  {pageRows.map((r, i) => (
                    <tr key={r.id ?? `${startIdx + i}`} className="hover:bg-black/[.02]">
                      ...
                      <td className="px-4 py-2">{r.prefix || "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
