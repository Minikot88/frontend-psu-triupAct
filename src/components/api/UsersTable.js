...
export default function UsersTable() {
  ...
  const filtered = useMemo(() => {
    ...
    out.sort((a, b) => {
      if (sort.startsWith("name")) {
        const cmp = String(a.fullname ?? "").localeCompare(String(b.fullname ?? ""), "th");
        return sort === "name-asc" ? cmp : -cmp;
      } else if (sort.startsWith("id")) {
        const na = Number(a.userId), nb = Number(b.userId);
        if (!isNaN(na) && !isNaN(nb)) return sort === "id-asc" ? na - nb : nb - na;
        ...
        const cmp = String(a.role ?? "").localeCompare(String(b.role ?? ""), "th");
        return sort === "role-asc" ? cmp : -cmp;
      }
      return 0;
    });
  ...
  return (
    <Card>
      <CardHeader title="ตารางผู้ใช้ /api/users" onToggle={() => setShowTable(!showTable)} isOpen={showTable} />

      {showTable && (
        <>
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input value={q} onChange={(e)=>{setQ(e.target.value);setPage(1);}} placeholder="ค้นหา ชื่อ/อีเมล/เลขบัตร/บทบาท/ID…" className="w-[28rem] rounded-xl border border-black/15 bg-white px-3 py-2 text-sm outline-none" />
              <select value={sort} onChange={(e)=>setSort(e.target.value)} className="rounded-xl border border-black/15 bg-white px-3 py-2 text-sm outline-none">
                ...
                <option value="role-asc">บทบาท (A→Z)</option>
                <option value="role-desc">บทบาท (Z→A)</option>
              </select>
